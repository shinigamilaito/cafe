...
xml.salida do
  ...
  xml.entregado_por @salida_proceso.entradas_afectadas.map(&:entregado_por).join("-")
  ...
  xml.cliente do
    xml.representante_legal @salida_proceso.entradas_afectadas.map() {|e| e.client.legal_representative}.join("-")    
    xml.organizacion @salida_proceso.entradas_afectadas.map() {|e| e.client.organization}.join("-")
  end
  xml.item_salidas do
